...
export default function OrderList() {
  ...
  return (
    <div className="w-full bg-white dark:bg-[#1C1C1C] mx-auto overflow-hidden">
      {/* Top Controls */}
      <div className="flex my-2 mx-3 items-center justify-between py-2 px-2 gap-2 flex-wrap bg-[#F7F9FB]">
        ...
        <input
          ...
          className="bg-gray-100 rounded-xl px-4 py-2 text-sm w-64 outline-none focus:ring-2 focus:ring-blue-300"
        />
      </div>
            ...
            <AnimatePresence>
              {paginated.length === 0 && (
                <motion.tr
                  ...
                  exit={{ opacity: 0 }}
                >
                  <td colSpan={8} className="p-10 text-center text-gray-400 font-semibold">
                    No Results
                  </td>
                </motion.tr>
              )}
        ...
        <nav className="inline-flex rounded-md -space-x-px" aria-label="Pagination">
          {[...Array(Math.ceil(filtered.length / rowsPerPage)).keys()].map((p) => (
            <button
              key={p}
              onClick={() => setPage(p + 1)}
              className={`relative inline-flex items-center px-3 py-1 border text-sm font-semibold focus:z-20 ${
                page === p + 1
